...
class ColumnParallelLinear(torch.nn.Module):
    ...
    def __init__(self, input_size, output_size, *,
        ...
        self.config = config
        self.disable_fp8_linear = disable_fp8_linear
        ...
        if self.training and (os.getenv('FP8_LINEAR', 'false').lower() == 'true') and (not self.disable_fp8_linear):
            from experimental.fp8_linear import FP8Linear
            self.output_parallel_linear = FP8Linear()
    ...
    def forward(self,
        ...
        if (args.transformer_impl == "transformer_engine" and get_accelerator().device_name() == 'hpu') \
            or ((os.getenv('FP8_LINEAR', 'false').lower() == 'true') and (not self.disable_fp8_linear)):
