...
class ParallelMLP(MegatronModule):
    ...
    def __init__(self, config, moe=False, enable_expert_tensor_parallelism=False):
        ...
        if args.openai_gelu:
            ...
            if os.getenv('SCALED_SWIGLU', 'false').lower() == 'true':
                from experimental.fp8_linear import ScaledSwiglu
                swiglu = ScaledSwiglu(delayed=os.getenv('DELAYED_SCALED_SWIGLU', 'false').lower() == 'true')
            else:
                def swiglu(x):
        ...
        self.dense_4h_to_h = tensor_parallel.RowParallelLinear(
            ...
            moe=moe,
            enable_expert_tensor_parallelism=enable_expert_tensor_parallelism,
            disable_fp8_linear=os.getenv('DISABLE_FP8_MLP_LINEAR2', 'false').lower() == 'true'
        )
    ...
    def forward(self, hidden_states):
        ...
        if self.bias_gelu_fusion:
            ...
            if os.getenv('SCALED_SWIGLU', 'false').lower() == 'true':
                intermediate_parallel, s = self.activation_func(intermediate_parallel)
            else:
