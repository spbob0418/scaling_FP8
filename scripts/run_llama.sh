...
# FP8 FLAGS
FP8_LINEAR=${HL_FP8_LINEAR:-"false"}
DISABLE_FP8=${HL_DISABLE_FP8:-"false"}
DISABLE_FP8_MLP_LINEAR2=${HL_DISABLE_FP8_MLP_LINEAR2:-"false"}
SCALED_SWIGLU=${HL_SCALED_SWIGLU:-"false"}
DETACH_SCALED_SWIGLU=${HL_DETACH_SCALED_SWIGLU:-"false"}
DELAYED_SCALED_SWIGLU=${HL_DELAYED_SCALED_SWIGLU:-"false"}
FP8_OPTIMIZER=${HL_FP8_OPTIMIZER:-"false"}
MW_DTYPE=${HL_MW_DTYPE:-"fp32"}
M1_DTYPE=${HL_M1_DTYPE:-"fp32"}
M2_DTYPE=${HL_M2_DTYPE:-"fp32"}
SFTZ=${HL_SFTZ:-"false"}
# ----------------------
...
if [ "$LLAMA_VER" = "1" ]; then
    ...
    if [ $LLAMA_MODEL_SIZE -eq 70 ]; then
        ...
        N_LAYERS=${HL_NUM_LAYERS:-32} # must be divisible by PP
        NHEADS=${HL_NUM_HEADS:-32} # must be divisible by TP
        NHIDDEN=$((128*$NHEADS)) # 4096
        NUM_KV_HEADS=${NHEADS} # must be divisible by TP
        FFN_HIDDEN_SIZE=11008
        LR=3e-4
        MIN_LR=3e-5
...
CMD="${CMD} \
    ...
    DISABLE_FP8_MLP_LINEAR2=$DISABLE_FP8_MLP_LINEAR2 \
    SCALED_SWIGLU=$SCALED_SWIGLU \
    DETACH_SCALED_SWIGLU=$DETACH_SCALED_SWIGLU \
    DELAYED_SCALED_SWIGLU=$DELAYED_SCALED_SWIGLU \
